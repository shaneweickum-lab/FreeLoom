    ...
    <div className="flex flex-col gap-6 h-[calc(100vh-12rem)]">
      <div>
        ...
        <p className="text-muted text-sm">
          Talk through {currentStudent.name}&apos;s learning out loud — the assistant can log activities, approve or
          fix courses, update the profile, and generate a transcript for you. It can&apos;t upload photos (use the
          Portfolio Builder for that) or add/remove children (use the Dashboard).
        </p>
      </div>

      <div className="flex-1 overflow-y-auto rounded-lg border border-border bg-surface p-4 flex flex-col gap-4">
          ...
          .map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] rounded-lg px-4 py-2.5 text-sm flex flex-col gap-2 ${
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          className="input"
          placeholder={`Tell the assistant about ${currentStudent.name}'s day, or ask for help…`}
          value={input}
