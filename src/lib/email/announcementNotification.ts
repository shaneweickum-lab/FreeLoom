/** An announcement notification email, sent only when the recipient's
 * school_profiles.email_notify_announcements is true and they have an email
 * on file. Same simplified template shape as messageNotification.ts. */
export function buildAnnouncementNotificationEmail({
  title,
  excerpt,
  appUrl,
}: {
        ...
        <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="width:480px; max-width:480px;">
          <tr>
            <td style="background-color:#141935; border:1px solid #2b3260; border-radius:16px; padding:32px 36px;">
              <p style="margin:0 0 8px; color:#b09cdc; font-size:11px; letter-spacing:0.08em; text-transform:uppercase;">Announcement</p>
              ...
              <a href="${appUrl}" style="display:inline-block; padding:10px 22px; background-color:#c7a252; color:#0a0d1c; font-size:13px; font-weight:700; text-decoration:none; border-radius:18px;">Open FreeLoom</a>
            </td>
          </tr>
          <tr>
